...
def testproj_wheel(sample_pkgs):
    return we_pe.WheelEntryPointExtractor(sample_pkgs.testproj_whl)


class TestWheelEntryPointExtractor:

    def test_no_entry_pts_text(self, mocker, sample_pkgs, testproj_wheel):
        mocker.patch.object(ZipFile, "namelist", return_value=[])
        with pytest.raises(ce.NoEntryPtsTxtFile):
            testproj_wheel.get_entry_points()

    def test_multiple_entry_pts_text(self, mocker, sample_pkgs, testproj_wheel):
        mocker.patch.object(ZipFile, "namelist", return_value=[
            "dir1/entry_points.txt", "dir2/entry_points.txt"])
        with pytest.raises(ce.MultipleEntryPtsTxtFiles):
            testproj_wheel.get_entry_points()

    def test_no_console_scripts(self, mocker, sample_pkgs, testproj_wheel):
        mocker.patch.object(entry_points_txt, "load", return_value={
            "console_scripts": {}
        })
