...
class VenvInstallPaths(NamedTuple):
    ...
    @classmethod
    def from_srepkg_path_and_orig_pkg_info(
            cls, srepkg_path: Path, orig_pkg_info: nt.OrigPkgInfo):
        venv_path = srepkg_path / (orig_pkg_info.pkg_name + '_venv')
        venv_py = venv_path / 'bin' / 'python'
        venv_pip = venv_path / 'bin' / 'pip'

        return cls(
            inner_src=orig_pkg_info.root_path,
            venv_path=venv_path,
            srepkg_root=srepkg_path,
            venv_py=venv_py,
            venv_pip=venv_pip)
...
class InnerPkgInstaller:
    ...
    def from_srepkg_path_and_orig_pkg_info(
            cls, srepkg_path: Path, orig_pkg_info: nt.OrigPkgInfo):
        return cls(VenvInstallPaths.from_srepkg_path_and_orig_pkg_info(
            srepkg_path, orig_pkg_info))

    def build_venv(self):
        env_builder = venv.EnvBuilder(with_pip=True)
        env_builder.create(self._paths.venv_path)
